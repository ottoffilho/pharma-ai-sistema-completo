...
-- =====================================================
-- 1. ADICIONAR CAMPOS DE MARKUP ÀS TABELAS EXISTENTES
-- =====================================================

-- Tabela insumos
ALTER TABLE insumos ADD COLUMN IF NOT EXISTS markup NUMERIC(6,2) DEFAULT 6.00;
ALTER TABLE insumos ADD COLUMN IF NOT EXISTS preco_venda NUMERIC(10,2);
ALTER TABLE insumos ADD COLUMN IF NOT EXISTS markup_personalizado BOOLEAN DEFAULT FALSE;
ALTER TABLE insumos ADD COLUMN IF NOT EXISTS data_ultima_atualizacao_preco TIMESTAMP WITH TIME ZONE DEFAULT NOW();

-- Tabela embalagens
ALTER TABLE embalagens ADD COLUMN IF NOT EXISTS markup NUMERIC(6,2) DEFAULT 6.00;
ALTER TABLE embalagens ADD COLUMN IF NOT EXISTS preco_venda NUMERIC(10,2);
ALTER TABLE embalagens ADD COLUMN IF NOT EXISTS markup_personalizado BOOLEAN DEFAULT FALSE;
ALTER TABLE embalagens ADD COLUMN IF NOT EXISTS data_ultima_atualizacao_preco TIMESTAMP WITH TIME ZONE DEFAULT NOW();
...
BEGIN
  ...
  IF NEW.preco_custo IS NOT NULL AND NEW.markup IS NOT NULL THEN
    NEW.preco_venda = ROUND(NEW.preco_custo * NEW.markup, 2);
...
BEGIN
  IF TG_OP = 'UPDATE' AND (
    OLD.preco_custo != NEW.preco_custo OR 
...
DROP TRIGGER IF EXISTS trigger_calcular_preco_venda_insumos ON insumos;
...
DROP TRIGGER IF EXISTS trigger_calcular_preco_venda_embalagens ON embalagens;
...
DROP TRIGGER IF EXISTS trigger_historico_preco_insumos ON insumos;
...
DROP TRIGGER IF EXISTS trigger_historico_preco_embalagens ON embalagens;
